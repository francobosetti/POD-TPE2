...
public class QueryCollator
        implements Collator<
                Map.Entry<CountyInfraction, Long>, List<Map.Entry<String, List<String>>>> {
    @Override
    public List<Map.Entry<String, List<String>>> collate(
            Iterable<Map.Entry<CountyInfraction, Long>> values) {
        ...
        for (Map.Entry<CountyInfraction, Long> entry : values) {
            ...
            countyMap
                    .computeIfAbsent(county, k -> new ArrayList<>())
                    .add(new CountyInfractionCount(entry.getKey().infraction(), entry.getValue()));
        }
        ...
        for (Map.Entry<String, List<CountyInfractionCount>> entry : countyMap.entrySet()) {
            ...
            List<String> value =
                    entry.getValue().stream()
                            .sorted(
                                    Comparator.comparingLong(CountyInfractionCount::count)
                                            .reversed())
                            .limit(3)
                            .map(CountyInfractionCount::infraction)
                            .toList();
